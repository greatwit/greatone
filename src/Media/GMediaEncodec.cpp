...
#define LOG_TAG "GMediaEncodec-JNI"
#include <utils/Log.h>

#include "GMediaEncodec.h"
...
GMediaEncodec::GMediaEncodec(const char *name, bool nameIsType, bool encoder)
			:mClass(NULL)
			,mObject(NULL)
			,mbRunning(false)
{
...
status_t GMediaEncodec::initCheck() const 
{
    return mCodec != NULL ? OK : NO_INIT;
}

GMediaEncodec::~GMediaEncodec() 
{
    if (mCodec != NULL) 
	{
        mCodec->release();
...
status_t GMediaEncodec::configure(
...
status_t GMediaEncodec::stop() 
{
    mSurfaceTextureClient.clear();
    return mCodec->stop();
}

status_t GMediaEncodec::flush() 
{
    return mCodec->flush();
}

status_t GMediaEncodec::queueInputBuffer( size_t index, size_t offset, size_t size, int64_t timeUs, uint32_t flags, AString *errorDetailMsg) 
{
    return mCodec->queueInputBuffer( index, offset, size, timeUs, flags, errorDetailMsg);
}


status_t GMediaEncodec::dequeueInputBuffer(size_t *index, int64_t timeoutUs) 
{
    return mCodec->dequeueInputBuffer(index, timeoutUs);
}

status_t GMediaEncodec::dequeueOutputBuffer(jobject bufferInfo, size_t *index, int64_t timeoutUs) 
{
    size_t size, offset;
    int64_t timeUs;
    uint32_t flags;
...
status_t GMediaEncodec::dequeueOutputBuffer( size_t *index, int64_t timeoutUs)
{
	size_t size, offset;
    int64_t timeUs;
    uint32_t flags;
...
status_t GMediaEncodec::releaseOutputBuffer(size_t index, bool render) 
{
    return render
...
status_t GMediaEncodec::getOutputFormat(jobject *format) const 
{
    sp<AMessage> msg;
    status_t err;
...
status_t GMediaEncodec::getBuffers(bool input, jobjectArray *bufArray) 
{
...
status_t GMediaEncodec::addBuffer(char*data, int len)
{
...
status_t GMediaEncodec::startCodec()
{
...
status_t GMediaEncodec::stopCodec()
{
	mbRunning = false;
	stop();
	return OK;
